...
- 🟡 **Renovate** — automate Docker image version bumps (replaces the dropped `watchtower`). **Runner live; pending the two Actions secrets + first run.** Runs as a scheduled **Forgejo Actions** job on a self-hosted runner (Renovate itself is ephemeral; the `forgejo-runner` v12.7.3 + isolated dind is the only long-lived add). **Deployed & verified:** `renovate.json` (4 custom managers + `docker`/`tika`/`dind` variant `packageRules`), the `forgejo-runner` ComposeApp + two-service template, its declarative `config.yaml` (`server.connections` UUID+token — runner v12.7+), `FORGEJO__actions__ENABLED` on forgejo, and `.forgejo/workflows/renovate.yml`. Runner is **Up** and "declared successfully" on docker_vm. Actions disabled on the ~155 `source-ag/*` repos (`disable-actions.sh`) so the instance-level runner only serves `daan/homelab`. **Remaining:** (1) set repo Actions secrets `RENOVATE_TOKEN` (`op://Homelab/Renovate/forgejo token`) + `GH_COM_TOKEN` (`op://Homelab/Renovate/github PAT`; named `GH_COM_TOKEN` not `GITHUB_COM_TOKEN` because Forgejo forbids the `GITHUB_` prefix — passed to Renovate via `RENOVATE_HOST_RULES`); (2) commit + push `renovate.json` and `.forgejo/workflows/renovate.yml` to the default branch so Forgejo sees the workflow; (3) trigger the workflow (`workflow_dispatch`) and review the Dependency Dashboard. Plan: [renovate-forgejo-actions.md](renovate-forgejo-actions.md). ([CLAUDE.md](../../CLAUDE.md) "Docker image versioning")
